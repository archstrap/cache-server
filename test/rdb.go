...
func NewRDBReader(path string) *RDBReader {
	...
	if err != nil {
		fmt.Printf("Unable to Set Up rdb file. Invalid RDB file %s provided", path)
		return nil
	}
...
func (r *RDBReader) ReadDb() (any, error) {
	...
	for {
		...
		case 0xFB:
			...
			fmt.Printf("expiry table size: %d\n", expiryTableSize)
		case 0xFD: // time stamp in milliseconds ( 4 byte unsigned integer) , LittleEndian
		...
		case 0xFC: // time stamp in seconds (8 byte unsigned long), LittleEndian
